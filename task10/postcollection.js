    ...
    export class PostCollection {
        ...
        addAll(posts) {
            ...
            for (let post of posts){
                if(PostCollection.validate(post) && typeof(this.get(post.id)) == "undefined") this._posts.push(post);
                else res.push(post);
            }
        ...
        static restore(key) {
            return new PostCollection(JSON.parse(localStorage.getItem(key), PostCollection.postReviver));
        }
        ...
        getPage(skip = 0, top = 10, filterConfig) {
            ...
            if (typeof filterConfig !== 'undefined' && 'createdAt' in filterConfig && typeof filterConfig.createdAt !== 'undefined')
        ...
        static validate(post) {
            ...
            if (post.id === '' || post.description.length > 280) return false;

            for (let str of post.likes) if (typeof (str) !== 'string') return false;

            for (let str of post.hashTags) if (typeof (str) !== 'string') return false;
        ...
        add(post) {
            ...
            post.id = String(this._posts.length + 1);
        ...
        edit(id, post) {
            ...
            for (let key in post)
                if (!(key in source) && key !== 'photoLink') {
            ...
            for (let key in post) {
                if (!['id', 'author', 'createdAt', 'likes', 'hashTags'].includes(key)) copy[key] = post[key];
            }

            if ('hashTags' in post) copy['hashTags'] = post.hashTags.slice();
        ...
        toggleLike(id, user) {
            if(typeof(id) !== "string" || typeof(user) !== "string") return false;
            ...
            if(ind === -1) post.likes.push(user);
            else post.likes.splice(ind, 1);
